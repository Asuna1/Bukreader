(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('bukreaderApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-bukreaderApp-error');
                var entityKey = httpResponse.headers('X-bukreaderApp-params');
                if (errorHeader) {
